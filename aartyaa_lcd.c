...
/** write a text to lcd */
static int aartyaa_lcd_write_text(const struct i2c_client  *t_client,
			const char *text_msg,
                        int msg_len)
{
                        ...
                        char *buf)
{
	...
	display_addr = aartyaa_pdata->display_text_addr;
...
static struct of_device_id aartyaa_lcd_match[] = {

	{ .compatible = "brcm,aartyaa_lcd" },
	{ },
}
MODULE_DEVICE_TABLE(of, aartyaa_lcd_match);

static struct i2c_driver aartyaa_lcd_driver = {
	.driver = {
		.name = "brcm,aartyaa_lcd",
		.owner = THIS_MODULE,
		.of_match_table = aartyaa_lcd_match,
	},
