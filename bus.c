...
int i2c_bus_add_device(struct device *dev)
{
	struct bus_type *bus = bus_get(dev->bus);
	int error = 0;

	printk("i2c_bus_add_device : bus = %d\n", bus);
	if (bus) {
		printk("bus: '%s': add device %s\n", bus->name, dev_name(dev));
		error = device_add_attrs(bus, dev);
		printk("i2c_bus_add_device : device_add_attrs error = %d\n", error);
		if (error)
			goto out_put;
